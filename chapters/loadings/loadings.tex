...
\end{align}
where $R$ is an arbitrary rotation matrix. While this provides a mathematical link between the probabilistic and non-probabilistic formulations of CCA, the correlation structure within each view ($\Sigma_{ii}$) means that the weights and loadings can be quite different in practice. The weights $U\sps{i}$ and loadings $W\sps{i}$ capture different aspects of the relationship between the views, with the loadings directly modeling how the latent variables manifest in each view's observations.
...
For brain-behavior relationships specifically, we can estimate likely SNR ranges based on domain knowledge. Measurement noise comes from scanner artifacts, physiological noise, and behavioral measurement error, while signal strength can be bounded using prior studies; for instance, heritability studies suggest upper bounds on brain-behavior relationships, as shared variance cannot exceed heritability estimates. These considerations suggest operating in the lower SNR regime of our simulations.
...
The absolute cosine similarity between two vectors is 1 if they are identical (up to a sign) and 0 if they are orthogonal.
...
\end{subfigure}
\caption{Bar plots of the true and estimated weights and \gls{loadings} for the $X$ data generated from the implicit latent variable models with sparse weights. The left column shows the results for the identity covariance matrices, while the right column shows the results for the correlated covariance matrices.}\label{fig:implicit-weights-loadings}
\end{figure}
...
\end{subfigure}
\caption{Bar plots of the true and estimated weights and \gls{loadings} for the $X$ data generated from the explicit latent variable models with sparse loadings. The left column shows the results for the identity covariance matrices, while the right column shows the results for the correlated covariance matrices.}\label{fig:explicit-weights-loadings}
\end{figure}
...
The unregularized CCA model is much worse than even the Ridge CCA model with the smallest regularization.
