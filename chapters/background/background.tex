...
\begin{figure}
    ...
    \tikz{
    % nodes
        \node[obs, minimum size=2cm, align=center] (y1) {Behaviour\\$y_1$}; % Behaviour node as y1
        \node[obs, left=of y1, minimum size=2cm, align=center] (x1) {Brain\\$x_1$}; % Brain node as x1
        \node[obs, below=of x1, minimum size=2cm, align=center] (x2) {Demographics\\$x_2$};
        \edge{x1} {y1}
        \edge{x2} {y1}}
...
\begin{align}
    ...
    \langle u^{(1)}, \Sigma_{12}u^{(2)} \rangle =u\spsT{1}\Sigma_{12}u\sps{2} = \sqrt{u\spsT{1}\Sigma_{11}u\sps{1}} \sqrt{u\spsT{2}\Sigma_{22}u\sps{2}} \cos(\theta)
\end{align}
...
As compared to \acrshort{pls}, \acrshort{cca} optimizes for the correlation between the two views, rather than the covariance.
...
\begin{align}
    ...
    \cos(\theta) = \frac{\langle u^{(1)}, \Sigma_{12}u^{(2)} \rangle}{\sqrt{\langle u^{(1)}, \Sigma_{11}u^{(1)} \rangle} \cdot \sqrt{\langle u^{(2)}, \Sigma_{22}u^{(2)} \rangle}} = \frac{u\spsT{1}\Sigma_{12}u\sps{2}}{\sqrt{u\spsT{1}\Sigma_{11}u\sps{1}} \sqrt{u\spsT{2}\Sigma_{22}u\sps{2}}}
\end{align}

By dividing by the norms of the representations, we can see that \acrshort{cca} is scale invariant, unlike \acrshort{pls}.
...
\subsubsection{Context: Eigenvalue Problems in CCA}
