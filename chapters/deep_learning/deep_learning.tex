...
\begin{align}
    \label{eq:DMCCA-full-batch}
    \LEY(\theta) = - \sum_{k=1}^K \lambda_k(\Sigma_{\hat{Z}}) = - \norm{\MCCA_K(\hat{Z})}_2^2
\end{align}
...
Self-Supervised Learning (SSL) has emerged as a powerful approach in the field of deep learning, particularly for tasks where labeled data is scarce.
A key concept in non-contrastive SSL is the creation of joint embeddings of augmented images, which involves generating different views of the same image and then learning to align their representations in a shared embedding space.
This process leverages inherent patterns and structures within the data, enabling the model to learn rich and useful feature representations without the need for explicit labeling.
A major challenge in non-contrastive SSL is avoiding the collapse of representations, which occurs when the model learns to ignore the input data and instead outputs a constant representation.
To address this problem, recent SSL methods have proposed the use of projectors, which are additional neural networks that map the representations to a higher-dimensional space.
This approach has been shown to prevent collapse and improve the quality of the learned representations.
