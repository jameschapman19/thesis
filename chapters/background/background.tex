...
Multiview machine learning encompasses a variety of techniques aimed at learning from data that have multiple sources or modalities, also known as `views'.
...
It comprises two distinct views: a brain modality and a behavioral modality.
...
However, it is essential to recognize that the presence of a common latent variable, inferred from these views, does not automatically imply causality in the observed data.

\subsubsection{Complementary and Redundant Information}
The nature of the information provided by different views (such as neuroimaging and behavioral data) is critical in multiview machine learning.

If these views contain redundant information about the latent variables, they can be used to denoise each other, enhancing the clarity and reliability of the data.
...
Conversely, if the views provide complementary information, they offer a richer and more complete understanding of the latent variables than either view alone.
In this case, the integration of diverse but related datasets leads to more robust and insightful models.
Complementary views expand our perspective, revealing aspects of the latent variables that are not apparent when views are considered in isolation.
...
In the context of multiview machine learning, self-supervised learning often operates under the assumption that different views are generated from a common source.
A natural pretext task, in this case, is to predict these source from the given views.
This not only enables the model to learn associations between views but also allows it to derive robust and informative representations for subsequent tasks like classification or regression.
...
Partial Least Squares (PLS)\citep{wold1975path} aims to maximize the shared covariance between two paired sets of data, referred to as `views". \acrshort{pls} can be seen as a generalization of \acrshort{pca}, where \acrshort{pca} becomes a special case when the two views are identical. The optimization problem for \acrshort{pls} can be formulated as:
...
In \acrshort{cca}, we aim to find the directions that maximize correlation, as opposed to maximizing covariance between two views of a dataset.
...
\section{Open challenges in \acrshort{cca}}
...
\subsubsection{\acrshort{kcca}}

Kernel Canonical Correlation Analysis (\acrshort{kcca}) extends \acrshort{cca} to capture nonlinear relationships.
