...
% Throughout this report we will refer to different modalities of data for the same subject as different "views", consistent with the literature \cite{sun2013survey} and with our hypothesis that they represent observations conditioned on a latent variable space. 

% Multiview machine learning models aim to learn useful representations from naturally paired datasets, such as images and sounds. Many multiview learning approaches also fall under the broader topic of unsupervised learning and can be used to generate robust low-dimensional representations for a downstream task such as classification \cite{zhao2017multi} or to discover relationships between views. While features from the different views could simply be concatenated for this purpose, multiview methods have been developed in order to take advantage of the differences between modalities with a view to improving model generalizability \cite{xu2013survey}.  They been widely applied across a range of fields such as neuroimaging \cite{Krishnan2011}, finance \cite{cassel2000measurement}, Imaging Genetics \cite{Hansen2021}, to find associations between views in large datasets.
...
In order to explain the methods that support CCA, it is helpful to first consider principal components analysis and partial least squares.
...
% \section{Generalized CCA: Extensions to more than 2 views}

% There are two methods in the literature for extending CCA and PLS to the case where we have three or more views in the data. Both of the methods optimise for the sum of pairwise correlations between each of the views and both collapse to CCA for the two view case.

% \subsubsection{Generalized (SUMCOR) CCA}\label{sec:sumcor}

% \begin{align}
%     &\max_{\bold{w_i}}\sum_i\sum_{j\neq i} \bold{w^{\top}_jX^{\top}_jX_iw_i}\\
%     &\text{subject to:} \notag\\
%     & \bold{w^{\top}_iX^{\top}_iX_iw_i}=1 \notag
% \end{align}

% In a similar way as to the original CCA, we can write this as a least squares optimisation:

% \begin{align}
%     &\max_{\bold{w_i}}\frac{1}{2}\sum_i\sum_j \|\bold{X_iw_i-X_jw_j}\|\\
%     &\text{subject to:} \notag\\
%     & \bold{w^{\top}_iX^{\top}_iX_iw_i}=1 \notag
% \end{align}

% If we also consider a similar problem with an additional variable $t$:

% \begin{align}
%     &\max_{\bold{w_i,t}}\sum_i \|\bold{X_iw_i-t}\|\\
%     &\text{subject to:} \notag\\
%     & \bold{w^{\top}_iX^{\top}_iX_iw_i}=1 \notag
% \end{align}
...
% \begin{align}
%     &f(\bold{w_i,t},\lambda)=\sum_i \bold{w^{\top}_iX^{\top}_iX_iw_i + t^{\top}t} - 2\bold{t^{\top}X_iw_i} +\lambda(\bold{w^{\top}_iX^{\top}_iX_iw_i}-1)
% \end{align}

% Which gives first order conditions:

% \begin{align}
%     &\bold{t}=\frac{\sum_i \bold{X_iw_i}}{n}\\
%     &\bold{w_i}=\frac{\bold{(X^{\top}_iX_i)^{-1}X^{\top}_it}}{1+\lambda}\\
%     &\bold{w^{\top}_iX^{\top}_iX_iw_i}=1
% \end{align}

% We can see that the optimal $\bold{t}$ is the average of the projections $\bold{X_iw_i}$. 

% \subsubsection{Generalized (MAXVAR) CCA}\label{sec:maxvar}

% Caroll's formulation of the generalized CCA adds an auxiliary vector so that the distance between all projections and the auxiliary vector are minimised. This can be summarised as the optimization problem \cite{van2011generalized}:

% \begin{align}
%     &\max_{w_i,t}\sum_i \bold{\frac{t^{\top}X_iw_i}{\|X_iw_i\|}}\\
%     &\text{subject to:} \notag\\
%     & \bold{t^{\top}t}=1 \notag 
% \end{align}

% In this formulation $\bold{t}$ is referred to as the group configuration. The problem is multiconvex so by forming the lagrangian we can find the optimal $\bold{t}$ as:

% \begin{align}
%     &\bold{t} = \bold{\frac{\sum_i X_iw_i}{\|\sum_i X_iw_i\|}}
% \end{align}

% Which is equivalent to finding the eigenvectors and values of the sum of the projection matrices:

% \begin{align}
% \bold{Q}=\sum_i \bold{X_i\left(X_i X_i^{\top}\right)^{-1} X_i^{\top}}
% \end{align}

% \subsubsection{Regularised Generalized CCA}

% Tenenhaus proposed a method for regularising Generalized CCA that is analagous to the regularised CCA described earlier in \ref{sec:Regularised CCA} for the generalized CCA objective function \cite{tenenhaus2011regularised}. They also include an additional set of parameters $c_{ij}$ that can be chosen to weight different pairs of views which can be either learnt or chosen a priori. Another change from standard CCA is the use of the functional $g(x)$. This function defines the problem and has been set to $g(x)=x$ \cite{kramer2007kernelizing}, $g(x)=x^2$ and $g(x)=\|x\|_1$ depending on the problem. 

% \begin{align}
%     & \bold{w_{opt}}=\underset{\bold{w}}{\mathrm{argmax}}\{ \sum_i \sum_{j\neq i} c_{ij} g(\bold{w_i^{\top}X_i^{\top}X_jw_j})  \}\\
%     & \text{subject to:} \notag\\
%     & (1-\tau_i)\bold{w_i^{\top}X_i^{\top}X_iw_i}+\tau_i\bold{w_i^{\top}w_i}=1 \notag\\
% \end{align}


% \section{Non-Linear CCA}
% \subsection{Kernel Canonical Correlation Analysis (KCCA)}

% The methods discussed so far only allow us to find linear relationships between datasets. Kernel Canonical Correlation Analysis is a non-parametric method for CCA \cite{hardoon2004canonical}. It can be expressed as the optimization problem:
...
% Where $\bold{K_i}$ denotes the kernel matrix for each view. Note that by substituting $\bold{w_i}=\bold{X^{\top}_i\alpha_i}$, if we use linear kernels so that $\bold{K_i}=\bold{X_iX_i^{\top}}$ we recover the normal CCA optimization problem. For this reason, the solution degenerates when the number of features is greater than or equal to the number of samples.

% In order to solve this problem, we can add regularisation either by adding regularisation to the alternating least squares optimisations or in the manner proposed by Hardoon, itself analagous to the CCA/PLS mixing we showed in section \ref{sec:Regularised CCA}.
...
% One benefit of using Kernel CCA with high-dimensional data ($p>n$), is that the kernel matrices $\bold{K_1}$ and $\bold{K_2}$ are $n\times n$ dimensional rather than $p\times p$ dimensional so that kernel CCA scales with the number of samples rather than the number of features.

% Furthermore, since we have not imposed any restriction on the kernel type, we can replace the linear kernel with any choice of kernel; with polynomial and radial basis function (rbf) kernels the most common choices in the literature.

% Since kernel CCA is non-parametric, when evaluating a KCCA model on out-of-sample data, we need to form a kernel $k(\bold{X_{train},X_{test}})$ and this requires us to access all of the training data at test time.
