...
SGHA has a computational complexity of $\mathcal{O}(k^2(d_1+ d_2))$ per iteration, which is higher than that of the stochastic power method but still potentially much faster than batch methods.
...
\subsubsection{Limitations of Existing Stochastic Algorithms}
...
\label{alg:batch-gep}
\begin{algorithmic}
\STATE {\bfseries Input:} Data $\X$, learning rate $(\eta_t)_t$, iterations $T$
...
To address the bias issue in the quadratic term, we use two independent mini-batches $\Z$ and $\Z'$. This allows us to define the following unbiased loss estimate:
...
To ensure robust results and account for statistical variations, we repeat the data generation and evaluation process five times for each feature size. This repetition allows us to compute average performance metrics and their associated variances, providing a more reliable basis for comparison.
...
\subsubsection{Evaluation Metric: Proportion of Correlation Captured (PCC)}
